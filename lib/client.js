window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			nav: "智能分流",
			title: "智能分流代理",
			description: "智能分流代理，自包含、上手简单，仅作用于 DeepSeek Harness 进程环境：国内流量直连，国外流量走订阅节点（hysteria2/vless-reality 经内置原生连接器）。",
			state: "状态",
			running: "运行中",
			starting: "启动中",
			stopped: "已停止",
		...
		const en = {
			nav: "Smart Routing",
			title: "Smart Routing Proxy",
			description: "A self-contained, zero-config rule-splitting proxy confined to the DeepSeek Harness process tree: domestic traffic goes direct, international traffic routes through subscription nodes (hysteria2/vless-reality via a bundled native connector).",
			state: "State",
			running: "Running",
			starting: "Starting",
			stopped: "Stopped",
